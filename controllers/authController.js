...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  await userFeedingSchema.create({
  ...
  if (!user) {
    ...
    if (!correctPassword) {
      ...
      if (process.env.NODE_ENV !== "testing") {
        return success(
          userId,
          ` just logged on to admin dashboard`,
          "Login",
          socket
        );
      }
...
exports.verify = catchAsync(async (req, res, next) => {
  ...
  if (elapsed > process.env.otpMinutesLimit) {
    ...
    user = await userSchema.findOneAndUpdate(
      { email: email },
      { $set: { otp: "", verified: true, otpExpiresIn: null } },
      { new: true }
    );
    ...
    await utilsSchema.updateMany(
      {},
      { $inc: { newStudentAlert: 1, numberOfUsers: 1, nonStudents: 1 } }
    );
...
exports.sendUserOTP = catchAsync(async (req, res, next) => {
  ...
  if (user) {
    ...
    await userSchema.findOneAndUpdate(
      { email: email },
      { $set: { otp: otp, otpExpiresIn: otpExpiresIn } },
      { new: true }
    );
  ...
  return (req, res, next) => {
    if (req.user.role === "bursar") {
      if (process.env.NODE_ENV === "production") {
        if (
          req.user.email === process.env.PRODUCTION_ADMIN_EMAIL ||
          req.user.email === process.env.DEVELOPMENT_ADMIN_EMAIL
        ) {
          next();
        } else {
          return next(
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    if (req.user) {
      ...
      req.user = req.user
        ? req.user
        : await restaurantSchema.findById(decoded.id);
      return next();
    }
  } catch (err) {
    return res.status(401).send({ success: false, message: "Invalid Token" });
...
exports.apiKeyVerification = catchAsync(async (req, res, next) => {
  ...
  const decoded = await apiKeySchema.findOne({
