...
exports.getAllUserData = catchAsync(async (req, res, next) => {
  ...
  const userFeedingData = await userFeedingSchema.findOne({
...
exports.updateUser = catchAsync(async (req, res, next) => {
  ...
  if (req.user.permissions) {
    if (
      req.user.permissions.some((value) =>
        ["all", "edit users"].includes(value)
      )
    ) {
    ...
    let user = await userSchema.updateOne(
      { _id: req.params.id, studentStatus: false },
      updateData,
      {
        new: true,
      }
    ...
    if (updateData.avatar) {
      user = await userSchema.updateOne(
        { _id: req.params.id },
