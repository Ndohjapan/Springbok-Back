...
exports.postRestaurant = catchAsync(async (req, res, next) => {
  ...
  let email = name.split(" ").slice(1).join("")
    ? name.split(" ")[0].toLowerCase() +
      name.split(" ")[1].toLowerCase() +
      "@lcu.edu.ng"
    : name.split(" ")[0].toLowerCase() + "@lcu.edu.ng";

  let avatar =
    "https://res.cloudinary.com/lcu-feeding-backup/image/upload/v1681319379/utils/restaurant_r7e2d5.png";
  ...
  res
    .status(200)
    .send({ status: true, message: "Restaurant Created", data: restaurant });
...
exports.getRestaurant = catchAsync(async (req, res, next) => {
  const food = await restaurantSchema
...
exports.updateRestaurant = catchAsync(async (req, res, next) => {
  ...
  let restaurant = await restaurantSchema
    .findOneAndUpdate({ id: req.params.id }, updateData, { new: false })
    .select("-balance -previousBalance");
  ...
  return success(
    userId,
    ` updated ${restaurant.name}: ${activityMessage}`,
    "Update",
    socket
  );
});
...
exports.deleteRestaurant = catchAsync(async (req, res, next) => {
  ...
  const food = await restaurantSchema.findByIdAndUpdate(req.params.id, {
    $set: { disabled: true },
  });
  ...
  return success(
    userId,
    ` deleted a restaurant from Database`,
    "Delete",
    socket
  );
});
...
exports.postFilter = catchAsync(async (req, res, next) => {
  ...
  let food = await restaurantSchema
    .find(updateData)
    .select("-balance -previousBalance");
...
exports.allTransactions = catchAsync(async (req, res, next) => {
  try {
    ...
    transactionSchema.paginate(
      { createdAt: { $gte: from, $lte: to }, disabled: false },
      options,
      function (err, result) {
        if (err) {
          return next(new AppError(err, 400));
