...
exports.getUsersDetails = catchAsync(async (req, res, next) => {
  ...
  let newUserAlert = await utilsSchema
    .find({})
    .select({ _id: 0, newStudentAlert: 1 });
  return res
    .status(200)
    .send({ status: true, message: "Successfull", userData, newUserAlert });
});

exports.getTransactionsDetails = catchAsync(async (req, res, next) => {
  let restaurantData = await restaurantTransactionsSchema.find({
    disabled: false,
  });
  let {
    totalAmountSpent,
    ...
    nonStudents,
    newStudentAlert,
  } = await utilsSchema.findOne({ _id: "6283ecbcffda06c1fd477266" });
  let transactionData = {
    totalAmountSpent,
    totalTransactions,
    totalDisbursedAmount,
  ...
  return res.status(200).send({
    ...
    message: "Succssful",
    restaurantData,
    transactoinsData: transactionData,
  });
});
...
exports.getDisbursementDetails = catchAsync(async (req, res, next) => {
  ...
  res
    .status(200)
    .send({ status: true, message: "Successful", data: disbursementData });
});
...
exports.createAdmin = catchAsync(async (req, res, next) => {
  ...
  const { firstname, lastname, email, password, number, role, permissions } =
    req.body;
...
exports.updateAdmins = catchAsync(async (req, res, next) => {
  ...
  let subAdmins = await adminSchema.findOneAndUpdate(
    { _id: req.params.id },
    updateData,
    { new: true }
  );
  ...
  res
    .status(200)
    .send({ status: true, message: "Sub Admin Updated", data: subAdmins });

  success(
    userId,
    ` updated ${subAdmins.firstname} ${subAdmins.lastname}'s: ${activityMessage}`,
    "Update",
    socket
  );
});
...
exports.getAllAdmins = catchAsync(async (req, res, next) => {
  ...
  return res
    .status(200)
    .send({ status: true, message: "Successful", data: admins });
});
...
exports.deleteAdmins = catchAsync(async (req, res, next) => {
  ...
  if (userId === req.params.id) {
    return res
      .status(403)
      .send({ status: false, message: "You Cannot Delete This Account" });
  } else {
...
exports.approveTempoararyTransactions = catchAsync(async (req, res, next) => {
  ...
  let userDetails = await tempoararyTransactionsSchema.aggregate(
    tempoararyTransactionAggregate
  );
  await tempoararyTransactionsSchema.updateMany(
    { to: restaurantId },
    { $set: { disabled: true } }
  );
    ...
    [
      {
        $set: {
          totalTransactions: {
            $add: ["$totalTransactions", "$manualTransactions"],
          },
          totalTransactionsAmount: {
            $add: ["$totalTransactionsAmount", "$manualTransactionsAmount"],
          },
        },
      },
    ],
...
exports.endSession = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    let disbursementData = await disbursementSchema.aggregate(
      disbursementAggregate
    );

    let transactionDetails = await transactionSchema.aggregate(
      restaurantAggregate
    );
    ...
    if (transactionDetails.length === 0) {
      return res
        .status(400)
        .send({ status: false, message: "No Transactions Within This Period" });
    }
    ...
    let userUpdate = userSchema.updateMany(
      {},
      { $set: { studentStatus: false } }
    );
    let feedingUpdate = userFeedingSchema.updateMany(
      {},
      {
        $set: {
          feedingType: 2,
    ...
    let newStudentAlert = utilsSchema.updateMany(
      {},
      { $set: { newStudentAlert: 0 } }
    );
    let restaurants = restaurantSchema.updateMany(
      {},
      { $set: { balance: 0, previousBalance: 0 } }
    );
...
exports.sendOtpToNewEmail = catchAsync(async (req, res, next) => {
  ...
  await tempoararyEmailSchema.create({ email, otp, otpExpiresIn });
...
exports.confirmOtpAndChangeEmail = catchAsync(async (req, res, next) => {
  ...
  let user = await tempoararyEmailSchema.findOne({ email: newEmail, otp: otp });
  if (!user) return next(new AppError(en.invalid_otp, 400));

  const currentDate = Date.now();
  const elapsed = dates.minuteDifference(currentDate, user.otpExpiresIn);

  if (elapsed > process.env.otpMinutesLimit) {
    return next(new AppError(en.expired_otp, 400));
  } else {
...
exports.unreadNotifications = catchAsync(async (req, res, next) => {
  ...
  return res.status(200).send({
    status: true,
    message: "Successful",
    unreadNotifications: utils[0].unreadNotifications,
  });
});
...
exports.allPermissions = catchAsync(async (req, res, next) => {
  ...
  return res.status(200).send({
    status: true,
    message: "Successful",
    permissions: utils[0].permissions,
  });
});
...
exports.resetStudentPin = catchAsync(async (req, res, next) => {
  ...
  await userFeedingSchema.findOneAndUpdate(
    { userId: userId },
